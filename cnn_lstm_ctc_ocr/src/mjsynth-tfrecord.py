...
#生成tfrecord数据块,图片数量num_shards,开始的编号start_shard
def gen_data(input_base_dir, image_list_filename, output_filebase, 
    ...
    # 配置会话
    #通过input_base_dir+image_list_filename来获取图片文件名，和txt
    ...
    #运算 图片的数量/输入的num_shards(float) 结果向上取整
    #每个shard含有的image比重，会出现部分shard是空的的情况

    for i in range(start_shard,num_shards):
        start = i*images_per_shard
        end = (i+1)*images_per_shard
        #开始后结束的图片伪索引（end-start==images_per_shard）
        ...
        #image_filenames[start:end] 不包括image_filenames[end]
        #生成tfrecord文件
    # Clean up writing last shard
    start = num_shards*images_per_shard #接在上面的循环下
    ...
    #上面是从start到end索引到的图片转换到一个tfrecord里，这是最后的一个，如果num_shards=0的话，就由有用，虽然并没乱用
    sess.close()
...
def gen_shard(sess, input_base_dir, image_filenames, output_filename,image_texts):
    ...
    for item,filename in enumerate(image_filenames):
        #item是索引号
        path_filename = os.path.join("../",filename)
        ...
        try:
            ...
            # labels 是将text通过out_charest转换的int列表
            if is_writable(width,text):
                #宽度大于最小值且string（text）比图像长
                 if len(labels)==0:
                     print(text,labels)
                 else:
                ...
                print('SKIPPING',filename) #不可写就跳过
        except:
...
def get_image_filenames(image_list_filename):
    ...
    with open(image_list_filename,encoding='UTF-8') as f:
        for line in f:
            ...
            file=line.split(' ',1)
            filename =file[0][3:] # split off "./" and number
            text=file[1].strip()
...
def get_image(sess,filename):
    ...
    with tf.gfile.FastGFile(filename, 'rb') as f: #二进制读取
        image_data = f.read()
...
def get_text_and_labels(text):
    ...
    labels = [out_charset.index(c) for c in list(text)] #通过out_charset字符集来索引编码
    return text,labels
...
def main(argv=None):
    gen_data('../data/images', 'label.txt', '../data/train/words',10)
